...
export function useMachine(stateMachine, options) {
    var service = interpret(createMachine(stateMachine.config, options ? options : stateMachine._options)).start();
...
export function useService(service) {
    var serviceRef = isRef(service)
        ? service
        : shallowRef(service);
    var state = shallowRef(serviceRef.value.state);
    watch(serviceRef, function (service, _, onCleanup) {
